...
class CmdCharacteristic(Characteristic):
    """
    CMD characteristic: receives UTF-8 command strings from the Android app.
    Commands are queued onto pi_controller._cmd_q for processing.
    Responses are sent back asynchronously via pi_controller.ble_send().
    """
    def __init__(self, bus, index, service, pi):
        super().__init__(bus, index, CMD_UUID, ["write"], service)
        self.pi = pi

    def WriteValue(self, value, options):
        # Convert bytes -> text command and queue it on the pi
        cmd = bytes(value).decode("utf-8", errors="replace")
        print(f"[BLE] CMD received: {cmd!r}")
        # Put command directly onto the pi's queue.
        # Do NOT notify here — the pi_controller will call ble_send() with the
        # real response once it has processed the command (e.g. config string).
        self.pi._cmd_q.put(cmd)


class RespCharacteristic(Characteristic):
    """
    RESP characteristic: sends responses/notifications to the Android app.
    Updated by pi_controller via ble_send(), which calls ble_send_data() on
    the GLib thread to set .value and emit PropertiesChanged.
    """
    def __init__(self, bus, index, service):
        super().__init__(bus, index, RESP_UUID, ["read", "notify"], service)
...
def run(pi_controller, name="SPT-Pi"):
    ...
    cmd  = CmdCharacteristic(bus, 1, svc, pi_controller)
    ...
    def start_advertising():
        ...
        pi_controller.set_state(State.BLUETOOTH_ON)
    ...
    def ble_stop_now():
        ...
        pi_controller.set_state(State.BLUETOOTH_OFF)

        return False
   
    def ble_send_data(data: str):
        resp.value = dbus.Array(data.encode("utf-8"), signature="y")
    ...
    def on_properties_changed(interface, changed, invalidated, path=None):
        ...
        # On first connection BlueZ creates the device object and sets Connected=True
        # in the same step. The PropertiesChanged signal sometimes doesn't include
        # "Connected" in the changed dict on this first emission — it's set as part
        # of object creation rather than a property change. So if "Connected" isn't
        # in changed, we still check the actual current value on the device object.
        if "Connected" not in changed:
            try:
                dev_props = dbus.Interface(
                    bus.get_object(BLUEZ_SERVICE_NAME, path), DBUS_PROP_IFACE
                )
                is_connected = bool(dev_props.Get(DEVICE_IFACE, "Connected"))
    ...
    # -----------------------
    def reg_app_cb():
        print("[BLE] GATT app registered — starting advertising")
        # Only start advertising AFTER RegisterApplication completes.
        # The call is async; advertising before this callback fires means phones
        # can connect and attempt writes before BlueZ knows about our characteristics,
        # causing immediate ATT WRITE_NOT_PERMITTED (status=1) on every write.
        start_advertising()
    ...
    pi_controller.ble_send  = lambda data: GLib.idle_add(ble_send_data, data)
    ...
    try:
        ...
        # Unregister all DBus object paths so they can be re-registered
        # if run() is called again (e.g. after bluetooth_toggle).
        for obj in [agent, app, svc, resp, cmd, adv]:
            try:
